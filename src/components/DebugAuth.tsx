...
export default function DebugAuth() {
  ...
  const testRepresentatives = async () => {
    try {
      ...
      console.log('Teste 1: URL completa http://localhost:3000/representatives');
      const response1 = await fetch('http://localhost:3000/representatives', {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const testEndpoints = async () => {
    try {
      ...
      const consumersResponse = await fetch('http://localhost:3000/consumers', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
      ...
      const generatorsResponse = await fetch('http://localhost:3000/generators', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
